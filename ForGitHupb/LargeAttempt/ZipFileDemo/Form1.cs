            ...
            if (result == DialogResult.OK)
            {
                ...
                foreach (ZipArchiveEntry entry in zip_Folder.Entries)
                {
                    ...
                    if (file_process.GetOnlyClass(entry))
                    {
                        ...
                        file_process.GetContent_class(file_process.DestinationFile(entry), entry);
                        ...
                        var getLayer = file_process.Layer(getSignature,_FileOwner);
                        file_process.SaveLayerProcess(getLayer, "features1.txt");

                       var digidtData =  file_process.ChangeType_To_Digid(getLayer, file_process.GetReturnTypeOfMethod());
                        file_process.SaveLayerProcess(digidtData, "Digid_features.txt");
